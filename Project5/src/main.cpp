...
GLRenderBuffer *g_render_buffer;
...
inline void renderTeapot() {
    ...
    g_render_buffer = new GLRenderBuffer();
    ...
    g_teapot_program->SetUniform(5, p_Ka);
    g_teapot_program->SetUniform(6, p_Kd);
    g_teapot_program->SetUniform(7, p_Ks);
    setTeapotModelViewProjectionMatrix();
    setupTeapotBuffers();
...
int main(int argc, char *argv[]){
    ...
    g_window = glfwCreateWindow(g_screen_width, g_screen_height, "Project 5 of cs6610", NULL, NULL);
    if (!g_window){
        glfwTerminate();
        exit(EXIT_FAILURE);
    }
